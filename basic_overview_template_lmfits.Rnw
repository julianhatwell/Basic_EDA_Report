...
for (var in cols[cols != y]) {
    ...
    if (any(y == num_vars)) {lm.var <- lm(fmla1, data = df)}
    if (any(y == fac_vars)) {lm.var <- glm(fmla1, data = df, family = binomial)}
    cfs[var,1] <- lm.var$coefficients[2]
    print(summary(lm.var))
}
fmla <- as.formula(paste0(y, "~", paste(cols[cols!=y], collapse = "+")))
if (any(y == num_vars)) {lm.vars <- lm(fmla, data = df)}
if (any(y == fac_vars)) {lm.vars <- glm(fmla, data = df, family = binomial)}
